...
#include <bookle_path_plan/path_util.h>
					...
					for(int t=0; t < DIRECTION; t++) {
						...
						switch (t) {
							...
							case path_util::RIGHT:
								nodeList[id]->addAdjNode(&nodeList[id_base + path_util::FORWARD], COST_ROTATE);
								nodeList[id]->addAdjNode(&nodeList[id_base + path_util::BACK], COST_ROTATE);
								break;
							case path_util::BACK:
								nodeList[id]->addAdjNode(&nodeList[id_base + path_util::RIGHT], COST_ROTATE);
								nodeList[id]->addAdjNode(&nodeList[id_base + path_util::LEFT], COST_ROTATE);
