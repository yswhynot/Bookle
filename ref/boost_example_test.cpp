...
bool maze::solve() {
  ...
  dist_map m_distance_map;
  boost::associative_property_map<dist_map> dist_pmap(m_distance_map);
  ...
  try {
    astar_search(boost::make_vertex_subset_complement_filter(m_grid, m_barriers), s, heuristic,
                 boost::weight_map(weight).
                 predecessor_map(pred_pmap).
                 distance_map(dist_pmap).
                 visitor(visitor) );
  } catch(GoalFoundException fg) {
    // Walk backwards from the goal through the predecessor chain adding
    // vertices to the solution path.
    for (vertex_descriptor u = g; u != s; u = predecessor[u])
      m_solution.insert(u);
    m_solution.insert(s);
    m_solution_length = m_distance_map[g];
    return true;
  }
